...
When enabled (requires **Key simulation**), the macro stops simply rotating the key list and instead simulates two human hands: time is divided into slices, each slice is assigned to one hand; **hands only switch when the actual note rate exceeds the Speed Threshold** — the actual rate is derived from note intervals (notes/minute); note that one 90° tile is a half-beat, so its actual rate is **2×** the tile BPM (45° tiles 4×, straight tiles 1×), and SetSpeed is naturally covered by the intervals. Below the threshold one hand keeps playing (a single-event slice = single-finger tapping, multi-event slices roll that hand's fingers), above it L/R alternate and each hand takes more events as the rate rises until its whole key set is used. Slice lengths follow the chart's **local speed** (including SetSpeed events; the speed of the *next* event's floor is used) and are quantized to the beat grid — at or below the **Speed Threshold** a slice is one local half-beat and never spans past the next note (floor: threshold half-beat), so slow fine-grid notes (45° tiles etc.) get one slice each; above it an integer multiple of it (`h·round(rate/limit)`). Key hold time is the configured L/R duration ratio × slice length. Cuts snap to event gaps (natural cluster boundaries) so doubles/chords are not split across hands and mergeable bursts are not switched early.
...
| Global · Speed Threshold (BPM) | 500 | Hand-switch threshold (range 50 ~ 2000, unit = notes/minute): stay on one hand at or below the actual note rate, alternate above it; also sets slice granularity above the threshold (slice ≥ multiple of the half-beat). 90° tiles count as tile BPM ×2. |
