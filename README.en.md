...
| Global · Speed Threshold (BPM) | 500 | Hand-switch threshold (range 50 ~ 2000, unit = notes/minute): reset to the starting (main) hand at or below the actual note rate, alternate above it; also sets slice granularity above the threshold (slice ≥ multiple of the half-beat). 90° tiles count as tile BPM ×2. |
...
| L/R Ratio | `0.8,0.8` | Press-duration ratio (0 ~ 1). Hold time is capped by “this piece's note span + smallest internal gap” (no stretch across a Pause) and floored at the “threshold folded base” (~40–80 ms, so even very fast runs show a visible keypress); hold notes are handled automatically. |
