...
> 7. **Press-duration base fix**: the human floor is now derived from the **actual note interval** (`GetNoteFoldedPressLength`) instead of the local floor BPM (`bpm×floor.speed`), plus a same-moment tolerance of `1e-3`. On uniform charts (e.g. a snowflake chart) the old code varied the hold between 39–73 ms because SetSpeed gives each tile a different `speed`, and the two keys of one double could differ by >10 ms; now every note (and both keys of a double/chord) in a uniform passage holds the same duration
> 8. **Docs**: README and EN/CN localization strings updated
