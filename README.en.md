...
> 2. **Technique simulator rewrite**: piece length is quantized to the local half-beat (`h·round(rate/limit)` above the threshold), removing the old octave-folding 2x jump while keeping cuts aligned with the beat grid; boundary cutting scores by “distance (under-length penalized 2x) − gap” so doubles/chords stay on one hand and mergeable bursts are not split; per-floor `floor.speed` uses the *next* event's floor (fixes SetSpeed causing piece-length mismatch and 3-1-1 fragmentation); when a slice holds more events than the hand has keys, it takes exactly maxK events so every key is used (extreme BPM no longer under-uses keys); hands alternate only when the **actual note rate** (derived from event intervals, notes/minute; a 90° tile is 2× the tile BPM) exceeds the threshold, and below it the hand resets to the starting/main hand (e.g. a single finger at low KPS, slow passages after a fast one return to main); empty pieces no longer flip hands; segment reset only when the key config changes (fixes the starting hand repeating at the beginning); press duration is capped by the piece's note span (no long hold across a Pause) and floored at a base folded from the actual note interval (~40–80 ms), plus a “Legacy press duration (1.3.0.30)” toggle to switch between the old folded short tap and the new slice-following length
...
> 8. **Balance multi-press across hands**: new setting. When one moment has more notes than a hand has keys, on = split evenly across both hands (6→R3 L3, 7→R4 L3, 8→R4 L4, odd extra to the main hand), off = main hand takes all its keys then the rest goes to the other hand (6→R5 L1)
> 9. **Docs**: README and EN/CN localization strings updated
...
| Global · Speed Threshold (BPM) | 500 | Hand-switch threshold (range 50 ~ 2000, unit = notes/minute): reset to the starting (main) hand at or below the actual note rate, alternate above it; also sets how many notes each hand taps in a row above the threshold (slice = note interval × round(rate/(2·threshold)), equivalent to the old half-beat multiple for 90° tiles). 90° tiles count as tile BPM ×2. |
...
| L/R Ratio | `0.8,0.8` | Press-duration ratio (0 ~ 1). Hold time is capped by “this piece's note span + smallest internal gap” (no stretch across a Pause) and floored at a base folded from the actual note interval (~40–80 ms, so even very fast runs show a visible keypress; uniform passages and doubles hold consistently); hold notes are handled automatically. |
