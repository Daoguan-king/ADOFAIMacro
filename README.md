...
开启后（需同时开启 **按键模拟**），宏不再简单轮转按键序列，而是模拟真人双手打谱：时间按"片"划分，每片交给一只手；**只有实际音符速率超过速度阈值时才会换手**——实际速率按音符间隔折算（音符/分钟），注意 90° 砖一块 = 半拍，其实际速率是砖 BPM 的 **2 倍**（45° 砖 4 倍、直线砖 1 倍），SetSpeed 变速同样按间隔体现。未超阈值时保持单手连续敲击（片内 1 个事件即单指连打，多个事件由该手多指滚动），超过阈值后左右手交替、每手拿到的片内事件数随速率增加，直至用满该手全部按键。片长按谱面**局部速率**（含 SetSpeed/变速事件，取下一事件所属层的速度）量化到节拍网格——不超过 **速度阈值** 时取局部半拍、且不超过到下一音符的间隔（下限为阈值半拍），慢速细网格音符（如 45° 砖）每音一片；超过时取半拍的整数倍（`h·round(速率/阈值)`）。按键保持时长为所设"左/右手时长"比例 × 该片长度。分片切点自动吸附到事件间隙（连打簇的自然分界），不会把快双押/和弦拆到两只手，也不会把可合并的连打簇提前换手。
...
| 起始手 | 右手 | 第一个时间片交给哪只手。 |
| 全局·速度阈值 (BPM) | 500 | 换手阈值（范围 50 ~ 2000，单位=音符/分钟）：实际音符速率不超过该值时保持单手敲击，超过后才左右换手；同时决定超阈值段的分片粒度（片长 ≥ 半拍的整数倍）。90° 砖实际速率 = 砖 BPM×2。 |
