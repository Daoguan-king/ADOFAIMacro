...
> 2. **手法模拟算法重写**：分片时长连续映射（`max(半拍, 60/(2×阈值))`），消除速率刚超阈值时的 2 倍跳变；切点按“距离 − 间隙”综合评分吸附事件边界（不拆双押/和弦、不误并三押）；逐层 `floor.speed` 局部速率；空片不再切换手；段边界仅在键位配置变化时重置（修复起始手连按两次）
...
开启后（需同时开启 **按键模拟**），宏不再简单轮转按键序列，而是模拟真人双手打谱：时间按"片"划分，每片交给一只手，左右交替；片长按谱面**局部速率**（含 SetSpeed/变速事件）连续计算，不短于 **速度阈值 (BPM)** 对应的半拍——谱面越快，片内事件越多，由单手多指连打承担。分片切点自动吸附到事件间隙（连打簇的自然分界），不会把快双押/和弦拆到两只手。
...
| 起始手 | 右手 | 第一个时间片交给哪只手。 |
| 全局·速度阈值 (BPM) | 500 | 双手换手速度上限：片长不短于该 BPM 对应的半拍（范围 50 ~ 2000）。谱面更快时保持该换手频率，由单手承担更多事件。 |
